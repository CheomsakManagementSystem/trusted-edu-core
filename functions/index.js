...
const resolveActorProfile = async (uid) => {
  ...
  const directSnap = await usersRef.doc(uid).get();
  const querySnap = await usersRef.where("uid", "==", uid).limit(2).get();
...
const requireAdminActor = async (req) => {
  ...
  const claimRole = String(decoded.role || "").toUpperCase();
  const hasAdminClaim = decoded.admin === true || claimRole === ADMIN_ROLE;
  if (hasAdminClaim) {
